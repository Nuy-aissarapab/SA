...
function Payment() {
  ...
  // เลือกสัญญาที่ “ล่าสุด” ต่อคน (ลองใช้ end_date ถ้ามี ไม่งั้นใช้ ID มากสุด)
  const pickLatestContract = (arr: ContractInterface[]): ContractInterface | undefined => {
    ...
    const valid = [...arr].sort((a, b) => {
      ...
      if (ad.isValid() && bd.isValid()) {
        if (ad.isBefore(bd)) return 1;  // มาก่อนอยู่หลัง
        if (ad.isAfter(bd)) return -1;
        return 0;
      }
      // ถ้า end_date ไม่ชัวร์ ใช้ ID มากสุดแทน
  ...
  const goEvidence = (row: PaymentInterface) => {
    if (!row?.ID) return;
    localStorage.setItem("current_payment_id", String(row.ID));
    navigate(`/Payment/Evidence?paymentId=${row.ID}`);
  };
  ...
  const adminColumns: ColumnsType<PaymentRow> = [
    ...
    // ถ้าต้องการปุ่มดูหลักฐาน:
    // {
    //   title: "หลักฐาน",
    //   key: "evidence_btn",
    //   render: (_: any, row: PaymentRow) => (
    //     <Button size="small" onClick={() => goEvidence(row)}>ดูหลักฐาน</Button>
    //   ),
    // },
  ];
