// pages/admin/Managecontracts.tsx
...
type ContractWithRenewal = ContractInterface & {
  ...
  Student_ID?: number;
  Room_ID?: number;          // <- เผื่อ backend ส่งฟิลด์นี้มาอยู่แล้ว
...
const Managecontracts = () => {
  ...
  const isRealDate = (s?: string | null) => !!s && dayjs(s).isValid() && dayjs(s).year() >= 1900;

  type RenewalStatus = "pending" | "approved" | "rejected" | undefined;
  const getRenewalStatus = (r: ContractWithRenewal): RenewalStatus =>
  ...
  const handleCreate = async (values: any) => {
    const payload: ContractCreatePayload = {
      ...
      Room_ID: values.Room_ID != null ? Number(values.Room_ID) : undefined,   // ✅ ส่ง Room_ID
    };
  ...
  const columns: ColumnsType<ContractRow> = [
...

    {
      ...
      render: (_: unknown, r: ContractRow) => {
        const text  = mapRenewalStatus(r.renewal_status, r.renewal_pending);
        const color = colorRenewalStatus(r.renewal_status, r.renewal_pending);
          ...
          render: (_: any, record: ContractRow) => (
            <Space>
              <Button
                type="link"
                onClick={() => {
                  // ✅ ต้องตั้ง editingId เพื่อเปิดโหมดแก้ไข และให้ปุ่ม "อัปเดต" ทำงาน
                  setEditingId(record.ID!);

                  // ✅ ดึง StudentID ให้ถูกต้อง (ห้ามใช้ record.ID)
                  const studentValue =
                    (record as any).StudentID ??
                    record.Student?.ID ??
                    (record as any).Student_ID;

                  // ✅ ดึง Room_ID เป็นค่าหมายเลข id ของห้อง (ตรงกับ Select ที่ตั้ง value เป็น id)
                  const roomId =
  ...
  return (
    ...
    <Space>
      ...
      <Button
        onClick={async () => {
          // ✅ validate แล้วส่งค่าไป handleUpdate
          const values = await form.validateFields();
          await handleUpdate(values);
        }}
        disabled={!editingId}               // ไม่มีรายการที่กำลังแก้ = ปิดปุ่ม
      >
        อัปเดต
      </Button>

            {/* ปุ่มยกเลิก เมื่ออยู่โหมดแก้ไข */}
            {editingId && (
              <Button
                onClick={() => {
                  form.resetFields();
